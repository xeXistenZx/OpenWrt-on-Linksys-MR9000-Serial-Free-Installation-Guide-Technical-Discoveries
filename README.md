...
### The Two Requirements for MR9000 to Boot Successfully

Getting OpenWrt to boot correctly on the MR9000 requires **two separate conditions to both be met simultaneously**:

1. **The kernel must have a working QCA8075 driver** — this depends on kernel version and switch architecture
2. **The Device Tree (DTS) must correctly describe MR9000 hardware** — this depends on whether the image was built specifically for MR9000 or reused from EA8300

This is a critical distinction. A kernel with full QCA8075 support will still fail if the Device Tree tells it the hardware is something different. Conversely, a correct MR9000 Device Tree won't help if the kernel driver doesn't support QCA8075 properly.

### The Architecture Difference

| Firmware Image | Kernel | Switch Architecture | QCA8075 Driver | MR9000 DTS | Boots on MR9000? |
|---|---|---|---|---|---|
| EA8300 22.x | 5.10 | swconfig (old) | ⚠️ Partial | ❌ Wrong (EA8300) | ❌ Fails |
| EA8300 23.x | 5.15 | DSA | ⚠️ Incomplete | ❌ Wrong (EA8300) | ❌ Fails |
| EA8300 24.x | 5.15 | DSA | ⚠️ Incomplete | ❌ Wrong (EA8300) | ❌ Fails |
| EA8300 25.x | 6.12 | DSA | ✅ Full | ❌ Wrong (EA8300) | ❌ Fails |
| **MR9000 22.x** | 5.10 | swconfig | ⚠️ Partial | ✅ Correct | ✅ **Boots!** |
| **MR9000 25.x** | 6.12 | DSA | ✅ Full | ✅ Correct | ✅ **Full support** |

**Key insight:** EA8300 25.x has a fully capable QCA8075 driver — but it still fails on MR9000 because the Device Tree describes EA8300 hardware, not MR9000. The kernel sees the wrong hardware description and the switch initialisation fails regardless of driver quality.

**Why MR9000 22.x boots despite only partial QCA8075 support:** The correct MR9000 Device Tree compensates — the switch initialises well enough for basic networking to function. This is why MR9000-specific 22.x is a viable bootstrap step.

**Why MR9000 25.x is the ideal target:** Both conditions are fully met — correct Device Tree AND full QCA8075 DSA driver support in kernel 6.12. This is the recommended final firmware.

**The bottom line:** Always use MR9000-specific firmware builds. EA8300 firmware will never work on MR9000 regardless of version, because the Device Tree is fundamentally wrong for this hardware.
...
> **📋 PRACTICAL NOTE**
> This method was not directly tested in practice during the investigation that produced this guide. It is logically deduced to work based on the following confirmed facts:
> - MR9000-specific 22.x firmware boots successfully on MR9000 ✅ (confirmed)
> - `fw_setenv kernsize 500000` works correctly from an OpenWrt shell ✅ (confirmed via DD-WRT shell)
> - MR9000-specific 25.x firmware boots successfully after kernsize is set to 500000 ✅ (confirmed)
>
> The actual installation documented in this guide used **Method B (DD-WRT bootstrap)** to set kernsize, which was discovered first before the MR9000-specific 22.x builds were found. Method A is the logical simplification of the same process — we went the hard way! 😄
>
> If you test Method A successfully (or encounter any issues), please open an issue to let us know so this note can be updated!
